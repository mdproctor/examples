...
class PromptQualityTest {
    ...
    void evaluate_all_profiles() throws Exception {
        var filterChars  = System.getProperty("eval.characters");
        var filterLayers = System.getProperty("eval.layers");
        var charFilter = filterChars != null
                         ? Set.of(filterChars.split(",")) : Set.<String>of();
        var layerFilter = filterLayers != null
                          ? Set.of(filterLayers.split(",")) : Set.<String>of();
        ...
        for (ProfileMode profile : ProfileMode.values()) {
            ...
            if (!layerFilter.isEmpty() && !layerFilter.contains(layerKey)) {continue;}
            ...
            for (AgentDescriptor desc : descriptors) {
                if (!charFilter.isEmpty() && !charFilter.contains(desc.agentId())) {continue;}
